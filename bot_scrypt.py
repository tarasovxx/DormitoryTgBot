...
from db import sql_start, add_value, add_user, add_order, print_products, get_info, checker, receive_method, add_suggestion, pay_method_db, add_check, get_check, delete_check, get_floor, add_campus, get_campus, add_choices, get_choices, add_order_new, add_money_for_user, get_promocode
...
async def begin(message: types.Message):
    ...
    sql_start()
    add_user(name, message.from_user.id)
    #greeting photo
...
async def text(message: types.Message):
    ...
    if message.text == "🍫 Продукты":
        ...
        # check = checker(ff[0][0])
        camp = ff[0][0].replace("_", " ")
        promo = "promo_" + str(message.from_user.id)
    ...
    #if user press this btn activate discount
    elif message.text == "💯 Промокод":
        #await Promocode.promocode.set()
        await bot.send_message(message.chat.id, "️💯 Введите промокод, для того, чтобы применить скидку")
    elif message.text.startswith("promo_"):
        code = message.text
        name = message.from_user.username
        promocodes = await get_promocode()
        promo_codes = [x[0] for x in promocodes if isinstance(x[0], str) and x[0].startswith("promo_")]
        your = "promo_" + str(message.from_user.id)
        if (code in promo_codes and code != your):
            curr_order = await get_choices(name)
            await add_money_for_user(name, curr_order[0][1] * 0.1)
            add_choices(name, curr_order[0][0], curr_order[0][1] * 0.95)
            await bot.send_message(message.chat.id, "✳️ Промокод активирован, скидка к стоимости применена!",
...
# async def activate_promocode(message: types.Message, state: Promocode.promocode):
...
async def add_to_basket(callback: types.CallbackQuery):
    ...
    if (choices[0][1] is not None and choices[0][0] is not None):
        final_price = choices[0][1]
        choices_curr = choices[0][0].split(",")
    else:
        choices_curr = []
        final_price = 0
    ...
    if callback.data.startswith("add_day_"):
        ...
        if callback.data.replace("add_day_", "") not in choices_curr:
            ...
            choices_curr.append(product[0][0] + "[Этаж: " + str(floor) + "]")
            ...
            floor = product[0][2]
            choices_curr_text = ",".join(choices_curr)
            if (choices_curr_text[0] == ','): choices_curr_text = choices_curr_text[1:]
            add_choices(name, choices_curr_text, final_price)
